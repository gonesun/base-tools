...
//	private static final Logger logger = LoggerFactory.getLogger(ExportExcelService.class);
	...
	public byte[] export(String fieldName) throws ToolBusinessException {
		try {
			InputStream inputStream = this.getClass().getResourceAsStream(fieldName);
		...
		for (int i = 0; i < allCount; i++) {
			if (i < allCount - 1) {
				...
				excelBaseInfoDto.workbook.setSheetOrder(
						excelBaseInfoDto.workbook.getSheetName(excelBaseInfoDto.workbook.getNumberOfSheets() - 1),
						i + 1);
			}
			...
			// 特殊字符屏蔽
			pageTitle = pageTitle.replaceAll(specialTitleChar, emptyString);
			StringBuilder newTitle = new StringBuilder(pageTitle);
			...
			excelBaseInfoDto.workbook.setSheetName(i, newTitle.toString());
			try {
	...
	public byte[] exportAll(List<Map<String, String>> headMapList, List<String> pageTitleList,
		...
		Map<String, Integer> titleMap = new HashMap<>();
		for (int i = 0; i < allCount; i++) {
			int sheetIndex = sheetIndexList.get(i);
			excelBaseInfoDto.workbook.cloneSheet(sheetIndex);
			excelBaseInfoDto.setConfigBySon(sheetIndex);
    ...
    public byte[] export(Map<String, String> headMap, List<?> dataList, String fieldName,
		...
		try {
			...
			for (int i = 0; i < excelBaseInfoDto.dataList.size(); i++) {
				setRowFromObject(i, excelBaseInfoDto, notSetLineHeight);
			}
			ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
			excelBaseInfoDto.workbook.write(byteArrayOutputStream);
            if (!StringUtil.isNullOrEmpty(sheetName)) {
                excelBaseInfoDto.workbook.setSheetName(0, sheetName);
            }
			byteArrayOutputStream.flush();
			byteArrayOutputStream.close();
...
//			logger.error("导出Excel失败" + JSONObject.toJSONString(ex));
	...
	public byte[] export(Map<String, String> headMap, List<?> dataList, String fieldName,
		...
		byte[] bytes = null;
		try {
			this.initWithUnFix(unFixColList, sheetIndex, excelBaseInfoDto,userMap);
			commitHead(headMap, excelBaseInfoDto);
			for (int i = 0; i < excelBaseInfoDto.dataList.size(); i++) {
...
//			logger.error("导出Excel失败" + JSONObject.toJSONString(ex));
	...
	private void init(List<String> unFixColList, int sheetIndex,
		...
		// 读取Excel模板数据
		try {
			if (excelBaseInfoDto.inputStream == null) {
				...
				excelBaseInfoDto.workbook = new HSSFWorkbook(excelBaseInfoDto.inputStream);
			}
		} catch (IOException ex) {
			throw new ToolBusinessException("80562", "读取Excel模板文件异常！", null, ex);
		}
...
//					logger.error(String.format("动态列【%s】在模板中未找到对应列", fieldName));
		...
		for (int i = excelBaseInfoDto.headUnFixColCount; i < excelBaseInfoDto.colCount; i++) {
            ...
            if (fileDataInfo.mergeRow) {
                excelBaseInfoDto.colMergeMap.put(i, new HashMap<>());
            }
		}
...
//				logger.warn(String.format("【%s】未找到对应数据源 ", fileDataInfo.fieldName));
	...
	// 获取sheet的一行，没有则新增
	private Row getRow(int index, ExcelBaseInfoDto excelBaseInfoDto, Integer... notSetLineHeight) {
	...
	// 数据填充Excel行
	public void setRowFromObject(int index, ExcelBaseInfoDto excelBaseInfoDto, Integer... notSetLineHeight)
			throws Exception {
		int rowNo = index + excelBaseInfoDto.headRowCount;
		// 获取一个行
		Row row = getRow(rowNo, excelBaseInfoDto, notSetLineHeight);
		// 获取要输出的数据
		...
		for (FileDataInfo fileDataInfo : excelBaseInfoDto.bodyConfig) {
			...
			if (!fileDataInfo.isErrorOrNoSuchMethod && !StringUtil.isNullOrEmpty(fileDataInfo.fieldName)) {
				...
				String methodName = "get" + pre.toUpperCase() + fieldName.substring(1);
				try {
					Object value = null;
					try {
...
//						logger.warn(String.format("【%s】未找到对应的【%s】方法 ", fileDataInfo.fieldName, methodName), ex);
					...
					if (fileDataInfo.mergeRow) {
                        ...
                        if (!mergeMap.containsKey(key)) {
                            ...
                            mergeMap.put(key, cellRangeAddressList);
                        } else {
                            List<CellRangeAddress> cellRangeAddressList = mergeMap.get(key);
...
//					logger.warn(String.format("【%s】数据读取异常 ", fileDataInfo.fieldName), ex);
	...
	public Object getFieldValue(Object obj, String field) throws NoSuchMethodException {
